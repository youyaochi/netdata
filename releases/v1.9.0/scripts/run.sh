...
# set mode [collector, proxy, central]
# ref: https://github.com/firehol/netdata/wiki/Replication-Overview
if [ "$NETDATA_MODE" = "COLLECTOR" ] && [[ $NETDATA_STREAM_DESTINATION ]] && [[ $NETDATA_STREAM_API_KEY ]]; then
	echo "Setting collector mode."
...
[web]
    mode = none
EOF
elif [ "$NETDATA_MODE" = "PROXY" ] && [[ $NETDATA_STREAM_DESTINATION ]] && [[ $NETDATA_STREAM_API_KEY ]]; then
...
EOF
elif [ "$NETDATA_MODE" = "CENTRAL" ]; then
